...
NOTE FOR PFE REPORT: this mock is intentionally simple. Document this as
part of your development process — building against a mock first, then
integrating the real LLM, is a legitimate and common engineering practice
(lets you test the deterministic parts of the system in isolation).
"""
...
def classify_category(patient_message: str, available_categories: list) -> str:
    ...
    return "general_pain"  # fallback, matches real service's fallback behavior
...
def extract_structured_answer(question_text: str, question_type: str, patient_message: str):
    ...
    if question_type == "yes_no":
        ...
        if any(w in text for w in NO_WORDS):
            return "no"
        # crude fallback: guess based on presence of negation
        return "no" if "not" in text or "n't" in text else "yes"
    ...
    if question_type in ("duration_minutes", "duration_days"):
        ...
        return 30 if question_type == "duration_minutes" else 1  # neutral default

    if question_type == "categorical":
        # For pediatric_specific's "mild/moderate/significant" and
        # routine_checkup's "checkup/follow_up/prescription"
